...
public class TweakIniList {
    ...
    private static final String defaultLoadOrderHeader = ";This is a list of Tweak Inis that will be loaded.";
    private static final String defaultLoadExclusionsHeader = ";This is a list of Tweak Inis to be ignored while building ASIS.esp.";

    public static final String TWEAKINIS_PATH = "TWEAKINIS_PATH";

    private static final String tweakIniDirectory = System.getenv(TWEAKINIS_PATH); // "TweakInis";
    private static final String commentDelimiter = ";";

    private final File[] filesInDirectory;
    private final ArrayList<File> tweakIniFiles = new ArrayList<>();
    private final ArrayList<INI_asis> tweakInis = new ArrayList<>();
    private final Collection<String> fileNames = new ArrayList<>();
    private final ArrayList<String> loadOrder = new ArrayList<>();
    private final ArrayList<String> loadExclusions = new ArrayList<>();
    private final Collection<IniSectionHead> tweakIniSections = new ArrayList<>();
    private final Map<IniSectionHead, Collection<IniData>> data = new TreeMap<>();
    ...
    private Map<String, Map<String, String>> keyValueData() {
        ...
        for (IniSectionHead currentSection : data.keySet()) {
            if (currentSection.getFormat().equals(IniDataFormat.KEY_VALUE)) {
                dataToReturn.put(currentSection.getName(), new TreeMap<>());
    ...
    private Map<String, Collection<String>> valueData() {
        ...
        for (IniSectionHead currentSection : data.keySet()) {
            if (currentSection.getFormat().equals(IniDataFormat.VALUE)) {
                dataToReturn.put(currentSection.getName(), new ArrayList<>());
    ...
    private void setLoadOrderFile() {
        if (!existsLoadOrderFile()) {
            loadOrderFile = new File(tweakIniDirectory + File.separator + "TweakIniLoadOrder.txt");
    ...
    private void setLoadExclusionsFile() {
        if (!existsLoadExclusionsFile()) {
            loadExclusionsFile = new File(tweakIniDirectory + File.separator + "TweakIniLoadExclusions.txt");
            ...
            if (!data.containsKey(currentSection)) {
                ...
                for (IniData currentNewEntry : newData) {
                    if (currentNewEntry.getFormat().equals(IniDataFormat.VALUE)) {
                        ...
                        Collection<IniData> oldDataToRemove = new ArrayList<>();
                        Collection<IniData> newDataToAdd = new ArrayList<>();
    ...
    private void loadTweakInis() {
        ...
        for (String currentIniName : loadOrder) {
            tweakIniFiles.add(new File(tweakIniDirectory + File.separator + currentIniName));
        }
    ...
    private boolean isValidToLoad(INI_asis ini) {
        ...
        if (plugin != null && SPDatabase.hasMod(new ModListing(plugin))) {
            isValid = true;
        }
    ...
    private void displayData() {
        for (IniSectionHead currentSection : data.keySet()) {
            StringBuilder displayString = new StringBuilder("[" + currentSection.getName() + "]\n");

            for (IniData currentData : data.get(currentSection)) {
                if (currentData.getFormat().equals(IniDataFormat.KEY_VALUE)) {
                    displayString.append(currentData.getKey()).append("=").append(currentData.getValue()).append("\n");
                } else if (currentData.getFormat().equals(IniDataFormat.VALUE)) {
                    displayString.append(currentData.getValue()).append("\n");
                }
            }

            display(displayString.toString());
        }
    }
    ...
    public enum TweakIniSectionHead {
        ...
        private final INI_asis.IniDataFormat format;
